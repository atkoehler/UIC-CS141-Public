...
int readData(vector<string> &data, const string &filename)
{
    ...
    // Open the file for reading and validate it is open
    ifstream inFS(filename);
    if(!inFS.is_open())
    {
        cout << "Error: Unable to open filename - " << filename << endl;
        return 0;
    }

    // Continuously read from buffer a word at a time.
    while(inFS >> value)
    {        
        counter++;
        ...
        // Only put valid data into collection.
        ...
        else
        {
            ...
            // We could break on the first invalid data, but
            // reading the rest of the potentially valid data is
